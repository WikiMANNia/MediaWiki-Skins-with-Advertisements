...
<?php
	...
	protected function renderPortals( $portals ) {
		...
		if ( !empty( $this->getDonationBox() ) ) {
			$portals = array_merge ( $portals, array( 'donations' => $this->getDonationBox() ) );
		}
		if ( !empty( $this->getFacebookBox() ) ) {
			$portals = array_merge ( $portals, array( 'facebook' => $this->getFacebookBox() ) );
		}
			...
			<div class="<?php echo $body_class ?>">
				...
				<ul>
					...
					if ( $hook !== null ) {
						...
						$skin = $this;
						wfRunHooks( $hook, array( &$skin, true ) );
					}
					?>
				</ul>
				<?php
				...
				case 'SEARCH':
					...
					<div id="p-search" role="search">
						...
						<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
							<?php
							if ( $this->config->get( 'VectorAdUseSimpleSearch' ) ) {
							?>
							<div id="simpleSearch">
								<?php
							} else {
